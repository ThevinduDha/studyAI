...
|---|---|---|---|
| `_id` | ObjectId | Primary Key | Auto-generated |
| `document` | ObjectId | Reference to `Document._id` | Required, indexed |
| `module` | ObjectId | Reference to `Module._id` | Required, indexed |
| `chunkIndex` | Number | Zero-based sequential position within the document | Required, integer >= 0 |
| `text` | String | Normalized, segmented text content | Required, non-empty, trimmed |
| `tokenCount` | Number | Estimated token count (~1.33 * word count) | Optional, integer >= 0 |
| `characterCount`| Number | Exact character length of chunk text | Required, integer >= 0 |
| `metadata` | Subdocument | Retrieval & provenance metadata | Subdocument with defaults |
| `metadata.originalName` | String | Original client-side PDF filename | Preserved for citation display |
| `metadata.pageStart` | Number | Starting source page (if detectable) | Nullable / Optional |
